...
public class SimpleBankingAppTest {
  ...
  // this test method (test case) verifies if the Withdraw feature works properly
    ...
    String accountNumber  = "5495-1291";

    // Setup phase
    double balanceBefore = mainApp.getBalance(accountNumber);
    double depositAmount = -50.00;

    // Exercise phase
    mainApp.addTransaction(accountNumber, depositAmount);
    double balanceAfter = mainApp.getBalance(accountNumber);
    assert balanceBefore + depositAmount == balanceAfter;
